...
const double pi=3.1415926; // se da sub forma literei q
...
//asta nu e functia mea, e luata de pe net
...
bool isInside(pair<int, int> startPos) {
...
void handelMouse(SDL_Event  &event) {
    float ratio = 0.05;
    if(event.type == SDL_MOUSEBUTTONDOWN) {
    ...
    else if(event.type == SDL_MOUSEMOTION && startPos != make_pair(-1, -1) && isInside(startPos)) {
        ...
        if(dx  < 0) F.a += ratio, F.b += ratio; //mergi in dreapta
        if(dx > 0) F.a -= ratio,F.b -= ratio; //mergi in stanga
        if(dy < 0) F.c -= ratio, F.d -= ratio; //mergi in sus
        if(dy > 0) F.c += ratio, F.d += ratio; //mergi in jos
